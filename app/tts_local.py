...
from .obs import TTS_TTF_SECONDS, TTS_BYTES_TOTAL
...
def _try_xtts(text: str, voice: Optional[str]) -> Optional[Tuple[bytes, int, str, str]]:
    ...
    try:
        ...
        return pcm, sr, "xtts", "real"
    except Exception as exc:  # pragma: no cover - optional dependency
        logger.warning("XTTS not available: %s", exc)
        return None


def _try_silero(text: str, voice: Optional[str]) -> Optional[Tuple[bytes, int, str, str]]:
    ...
    try:
        ...
        return pcm, _silero_sr, "silero", "real"
    except Exception as exc:  # pragma: no cover - optional dependency
        logger.warning("Silero TTS not available: %s", exc)
        return None


def _sine_fallback(text: str, sample_rate: int = 16_000) -> Tuple[bytes, int, str, str]:
    ...
    return bytes(buf), sample_rate, "sine", "mock"


def synthesize(text: str, voice: Optional[str] = None) -> Tuple[bytes, int, str, str]:
...
def stream_bytes(
    ...
    """Yield ``data`` in small chunks and record metrics."""
    first = True
    for i in range(0, len(data), chunk_size):
        chunk = data[i : i + chunk_size]
        if first:
            TTS_TTF_SECONDS.labels(engine=engine, mode=mode).observe(
                time.time() - start_time
            )
            first = False
        TTS_BYTES_TOTAL.labels(engine=engine, mode=mode).inc(len(chunk))
        yield chunk
